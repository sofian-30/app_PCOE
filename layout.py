...
# Fichier layout "principal" : on retrouve le composant "page-content" géré dans le fichier "callbacks.py" pour faire afficher la page adéquate.
layout_template = html.Div([
    dcc.Location(id='url2', refresh=False), #permet de stocker l'URL de la page affichée (pour le callbacks de gestion des applis)
            ...
            dbc.CardHeader(
                dbc.CardLink(
                    html.H1(list_app["name"].loc[list_app["ind"]==i].iloc[0],
                     style={"color":"#024c97","font-weight":"bold", "font-size":"1.65vh", #change color => Belongs csv file ? 
                        "padding-top":"15px"}),
...
# -------- HEADER --------- #
    ...
    dbc.Row([
        dbc.Col([   
            dbc.Navbar([
                    dbc.Col([html.Img(src='/assets/logo_seenovate.png', height="40px",style={'margin-left':'1vw'})],xs=2,sm=2,md=2,lg=2,xl=2),
                    dbc.Col([html.Div(dbc.NavbarBrand("Titre page d'accueil", id="titre"),style={"color":"grey", "font-size":"1.65vh","textAlign":"center"}) #,className="text-white"
                    ],xs=8,sm=8,md=8,lg=8,xl=8,align="center"),
                    dbc.Col([html.Div(html.Img(src=app.get_asset_url('user2.png'), height="40px"))],xs=1,sm=1,md=1,lg=1,xl=1,align="right"),
                    dbc.Col([dbc.NavbarBrand("Logo Client")],xs=1,sm=1,md=1,lg=1,xl=1,style = {"color":"grey", "font-size":"1.65vh"},align="center") #className="ml-5 text-white"
            ],style = {"background-image": "-webkit-gradient(linear, right bottom, left top, from(#80DEEA), to(#1A77BD))"})
...
# --------- BODY --------- #
    ...
    dbc.Row(
        dbc.Col(
            html.Img(src=app.get_asset_url("Image3.jpeg"),style={"border-radius": "20% / 50%","height":"42vh","width":"102vh"}),
            width=12
        ),
